                    ...
                    <div class="flex">
                        ...
                        <div class="flex-shrink-0 flex items-center">
                            <inertia-link :href="route('dashboard')">
                                <avatar :user="$page.user" :size="60"></avatar>
                            </inertia-link>
                        </div>
                        ...
                        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                            <jet-nav-link :href="route('account', authUser.username)" :active="route().current('account')">
                                Account
                            </jet-nav-link>
                        </div>
                        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                            ...
                            <jet-dropdown align="right" width="48">
                                <template #trigger>
                                    <button v-if="$page.jetstream.managesProfilePhotos" class="flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition duration-150 ease-in-out">
                                        <img class="h-8 w-8 rounded-full object-cover" :src="authUser.profile_photo_url" :alt="authUser.name" />
                                    </button>
                                ...
                                <template #content>
                                    ...
                                    <template v-if="$page.jetstream.hasTeamFeatures">
                                        ...
                                        <!-- Team Settings -->
                                        <jet-dropdown-link :href="route('teams.show', authUser.current_team)">
                                            Team Settings
                                        </jet-dropdown-link>
                                        ...
                                        <template v-for="team in authUser.all_teams">
                                            <form @submit.prevent="switchToTeam(team)" :key="team.id">
                                                <jet-dropdown-link as="button">
                                                    <div class="flex items-center">
                                                        <svg v-if="team.id == authUser.current_team_id" class="mr-2 h-5 w-5 text-green-400" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                        <div>{{ team.name }}</div>
                                                    </div>
                                                </jet-dropdown-link>
                                            </form>
                ...
                <div class="pt-4 pb-1 border-t border-gray-200">
                    <div class="flex items-center px-4">
                        <div class="flex-shrink-0">
                            <img class="h-10 w-10 rounded-full" :src="authUser.profile_photo_url" :alt="authUser.name" />
                        </div>
                    ...
                    <div class="mt-3 space-y-1">
                        ...
                        <template v-if="$page.jetstream.hasTeamFeatures">
                            ...
                            <!-- Team Settings -->
                            <jet-responsive-nav-link :href="route('teams.show', authUser.current_team)" :active="route().current('teams.show')">
                                Team Settings
                            </jet-responsive-nav-link>
