<template>
  <div class="flex">
    <form method="POST" v-bind:action="'/thoughts/' + thought.id + '/like'">
      <input type="hidden" name="_token" :value="csrf">

      <div class="flex items-center mr-4">
        <svg viewBox="0 0 20 20" v-bind:class="{'text-blue-500': isThoughtLiked, 'text-gray-500': !isThoughtLiked}" class="text-gray-500 mr-1 w-3">
        ...
        <button type="submit" class="text-xs text-gray-500">{{thought.likes ? thought.likes : 0}}</button>
      </div>
    </form>
    <form method="POST" v-bind:action="'/thoughts/' + thought.id + '/like'">
      ...
      <div class="flex items-center">
        <svg viewBox="0 0 20 20" v-bind:class="{'text-blue-500': isThoughtDisliked, 'text-gray-500': !isThoughtDisliked} " class="w-3 mr-1">
          <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
            <g class="fill-current">
        ...
        <button type="submit" class="text-xs text-gray-500">{{thought.dislikes ? thought.dislikes : 0}}</button>
      </div>
    </form>
  </div>
</template>
